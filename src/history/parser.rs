...
mod tests {
    ...
    #[test]
    fn extracts_cwd_from_latest_user_message() {
        // The parser uses the *latest* cwd, not the first. This fixture has
        // two different cwd values; the last one should win.
        let content = [
        ...
        assert_eq!(
            conv.cwd,
            Some(PathBuf::from("/other/path")),
            "Should extract cwd from latest user message"
        );
    }
    ...
    fn extracts_the_last_user_question() {
        ...
        // Note: the empty-vec branch (`user_messages.last().cloned()` returning
        // `None`) is a type-system guarantee — it cannot produce `Some(...)`.
    }

    #[test]
    fn last_user_question_is_none_when_no_user_messages() {
        // A conversation with only assistant messages has no user content in
        // `preview_parts`, so the parser returns `Ok(None)` (filtered out).
        // This is the contract: assistant-only files produce no conversation,
        // so there is no way for an assistant message to appear as
        // `last_user_question`. If the parser ever changes to return `Some(conv)`
        // for assistant-only files, the `.expect(...)` below will document the
        // new contract that must be verified.
        let path = std::env::temp_dir().join(format!(
            "ccmanager-last-user-q-empty-{}.jsonl",
            std::process::id()
        ));
        let body = format!("{}\n", assistant_msg("just an assistant reply"));
        std::fs::write(&path, body).unwrap();

        let result = process_conversation_file(path.clone(), None, None).unwrap();
        std::fs::remove_file(&path).ok();

        assert!(
            result.is_none(),
            "assistant-only file should be filtered to Ok(None), got a conversation with {:?}",
            result.map(|c| c.last_user_question)
        );
    }
    ...
    fn latest_cwd_wins_when_session_moves() {
        ...
        let path = std::env::temp_dir().join(format!(
            "ccmanager-cwd-latest-wins-{}.jsonl",
            std::process::id()
        ));
        let body = format!(
            "{}\n{}\n",
            user_msg(
                "before rename",
                Some("/Users/me/Documents/Git/claude-history")
            ),
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/Users/me/Documents/Git/ccmanager"
            )),
