...
///
/// For worktree paths like `/Users/raine/code/myproject__worktrees/my-branch`,
/// returns `myproject/my-branch` to show both the main project and worktree name.
///
...
/// # Examples
/// ```
/// # use ccmanager::history::path::encoded_project_root;
/// assert_eq!(
...
mod tests {
    ...
    fn encodes_worktree_with_double_underscore() {
        let path = Path::new("/Users/raine/code/myproject__worktrees/feature-branch");
        let converted = convert_path_to_project_dir_name(path);
        assert_eq!(
            converted,
            "-Users-raine-code-myproject--worktrees-feature-branch"
        );
    }
    ...
    fn decode_preserves_dashes_in_folder_names_in_fallback() {
        ...
        let encoded = "-Users-raine-code-my-project";
        let decoded = decode_with_double_dash_as(encoded, "__");
        // This incorrectly decodes to /Users/raine/code/my/project
        // because single dashes are treated as path separators
        assert_eq!(decoded, "/Users/raine/code/my/project");
    }
