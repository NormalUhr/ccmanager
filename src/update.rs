...
const REPO: &str = "NormalUhr/ccmanager";
const BIN_NAME: &str = "ccmanager";
...
pub fn run() -> Result<()> {
    ...
    if is_homebrew_install(&canonical_exe) {
        return Err(AppError::UpdateError(
            "ccmanager is managed by Homebrew. Run `brew upgrade ccmanager` instead.".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_is_homebrew_cellar() {
        assert!(is_homebrew_install(Path::new(
            "/opt/homebrew/Cellar/ccmanager/0.1.42/bin/ccmanager"
        )));
    }
    ...
    fn test_is_homebrew_prefix() {
        assert!(is_homebrew_install(Path::new(
            "/usr/local/Cellar/ccmanager/0.1.42/bin/ccmanager"
        )));
    }

    #[test]
    fn test_is_not_homebrew_local_bin() {
        assert!(!is_homebrew_install(Path::new("/usr/local/bin/ccmanager")));
    }

    #[test]
    fn test_is_not_homebrew_home() {
        assert!(!is_homebrew_install(Path::new(
            "/home/user/.local/bin/ccmanager"
        )));
    }
}
