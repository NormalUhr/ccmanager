...
pub enum Commands {
    /// Update ccmanager to the latest version
    Update,
    /// Run as an MCP (Model Context Protocol) stdio server.
    /// Designed to be spawned by Claude Code; `ccmanager mcp` on its
    /// own will sit waiting for JSON-RPC on stdin.
    #[cfg(feature = "mcp")]
    Mcp,
    /// Run a local web UI on http://127.0.0.1:<port>
    #[cfg(feature = "serve")]
    Serve {
        /// Bind host. Non-localhost requires --token.
        #[arg(long, default_value = "127.0.0.1")]
        host: String,
        /// Bind port
        #[arg(long, short = 'P', default_value_t = 7878)]
        port: u16,
        /// Auth token (required when host is not 127.0.0.1)
        #[arg(long)]
        token: Option<String>,
        /// Open the default browser after starting
        #[arg(long)]
        open: bool,
        /// Disable mutation endpoints (rename/delete)
        #[arg(long)]
        read_only: bool,
    },
}
...
pub struct Args {
    ...
    /// Resume with the standard permission flow (omit
    /// --dangerously-skip-permissions). The default is to skip permissions;
    /// pass this when you want Claude to ask before each action.
    #[arg(
        long,
        help = "Resume with permission prompts (override the skip-permissions default)",
        requires = "resume"
    )]
