...
pub struct ResumeConfig {
    ...
    /// Whether the primary resume binding (Ctrl+R) appends
    /// `--dangerously-skip-permissions`. Defaults to true. Setting this to
    /// false flips the semantics: Ctrl+R becomes the standard
    /// "ask-before-acting" path, and the alt binding (Alt+R by default)
    /// becomes the skip-permissions path.
...
pub struct KeysConfig {
    ...
    /// Alternative resume binding — when pressed, behaves opposite to
    /// `resume` w.r.t. `--dangerously-skip-permissions`. Default: Alt+R.
    pub resume_alt: Option<KeyBinding>,
    /// Alternative fork-resume binding. Default: Alt+F.
    pub fork_alt: Option<KeyBinding>,
}
...
pub struct KeyBindings {
    ...
    pub resume_alt: KeyBinding,
    pub fork_alt: KeyBinding,
}
    ...
    pub fn from_config(config: Option<KeysConfig>) -> Self {
        ...
        match config {
            ...
            Some(cfg) => Self {
                ...
                resume_alt: cfg.resume_alt.unwrap_or(defaults.resume_alt),
                fork_alt: cfg.fork_alt.unwrap_or(defaults.fork_alt),
            },
        }
    }
}

/// Returns the path to the configuration file: ~/.config/ccmanager/config.toml
