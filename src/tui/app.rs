...
pub enum Action {
    Select(PathBuf),
    Delete(PathBuf),
    /// Resume the selected conversation. `skip_permissions` controls whether
    /// `--dangerously-skip-permissions` is passed to `claude`. Decided at
    /// keypress time by combining the configured default with which binding
    /// (primary vs alt) the user pressed.
    Resume {
        path: PathBuf,
    ...
    /// Reload the conversation list from disk and re-render the current
    /// viewer (if any). Handled inside the TUI loop — never bubbles out
    /// of `run_with_loader`.
    Refresh,
    Quit,
}
...
pub enum DialogMode {
    ...
    ConfirmDelete,
    /// Format-picker menu for copying the conversation to the clipboard.
    /// Triggered by both `e` and `y` (the latter only outside message-nav mode).
    YankMenu { selected: usize },
    /// Renaming the current conversation; `buffer` is the in-progress input
    RenameInput { buffer: String },
...
pub struct ViewState {
    ...
    /// When true, the viewer shows only top-level user questions, hiding
    /// Claude's answers, tool calls, and subagent internals.
...
pub struct App {
    ...
    keys: KeyBindings,
    /// Default for the primary resume binding: whether `--dangerously-skip-
    /// permissions` is passed to `claude`. The alt binding inverts this for
    /// a single resume. Sourced from `[resume] skip_permissions` in config.
    ...
    /// Path to re-select once `finish_loading()` runs. Set by
    /// `begin_refresh()` to remember the conversation the user was looking
    /// at before the refresh, so we can land them back on it once the
    /// re-scan completes (assuming the file still exists and passes the
    /// active filter).
...
impl App {
    ...
    pub fn new(
        ...
        let mut initial_list_state = ListState::default();
        initial_list_state.select(selected);
        Self {
            conversations,
            searchable,
            filtered,
            ...
            list_state: RefCell::new(initial_list_state),
        }
    }
    ...
    /// Whether `Ctrl+R` (the primary resume binding) appends
    /// `--dangerously-skip-permissions`. The alt binding inverts this.
    ...
    pub fn finish_loading(&mut self) {
        ...
        // Restore selection by path after a refresh, if the previous target
        // still exists in the filtered list. Falls back silently to the
        // first filtered entry (already set above) if the conversation was
        // deleted or filtered out by the active query.
        if let Some(target) = self.pending_selection_path.take()
            && let Some(filtered_idx) = self
                .filtered
                .iter()
                .position(|&i| self.conversations[i].path == target)
        {
    ...
    /// Reset to loading state so the main loop can re-populate the list
    /// from a fresh `load_all_conversations_streaming` receiver. Preserves
    /// the search query, workspace filter, and the path the user was
    /// looking at (which `finish_loading` will re-select if still present).
    ///
    /// View mode is NOT exited — the caller is expected to also call
    /// [`Self::re_render_view`] so the viewer immediately picks up any new
    /// content appended to the conversation's `.jsonl` since it was opened.
    pub fn begin_refresh(&mut self) {
        // Remember what the user was looking at. In view mode the viewer
        // path takes priority — that's the conversation they're actively
        // reading and want to return to after the rescan.
        ...
        self.pending_selection_path = target;

        // Drop the old list. `finish_loading` will reapply the active
        // query / workspace filter against the fresh data.
    ...
    /// Set a status message from outside the impl block (used by the event
    /// loop's resume-in-new-terminal handler).
    ...
    /// Return the encoded project directory name for the current workspace, if any.
    pub fn current_project_dir_name(&self) -> Option<&str> {
    ...
    fn handle_menu_key(&mut self, code: KeyCode) -> Option<Action> {
        ...
        match code {
            ...
            KeyCode::Char('1') => {
                self.perform_export(0);
                self.dialog_mode = DialogMode::None;
                None
            }
            KeyCode::Char('2') => {
                self.perform_export(1);
                self.dialog_mode = DialogMode::None;
                None
            }
            KeyCode::Char('3') => {
                self.perform_export(2);
                self.dialog_mode = DialogMode::None;
                None
            }
            KeyCode::Char('4') => {
                self.perform_export(3);
                self.dialog_mode = DialogMode::None;
                None
            }
    ...
    /// Copy the full conversation to the clipboard in the chosen format.
    ///
    /// Filters out intermediate steps (tool calls, tool results, thinking,
    /// narration between tool uses, subagents) and emits only the user prompt
    /// and Claude's final answer per round. The viewer's `t`/`T` toggles do
    /// not affect this clipboard copy.
    fn perform_export(&mut self, option: usize) {
        let path = match &self.app_mode {
        ...
        let result = crate::tui::export::export_to_clipboard(&path, format);

        self.status_message = Some((result.message, std::time::Instant::now()));
    }

    /// Handle a keypress while the rename modal is open.
    ///
        ...
        match code {
            ...
            KeyCode::Enter => {
                let title = match &self.dialog_mode {
                    DialogMode::RenameInput { buffer } => buffer.trim().to_string(),
                    _ => return None,
                };
                self.dialog_mode = DialogMode::None;
            ...
            KeyCode::Backspace => {
                if let DialogMode::RenameInput { buffer } = &mut self.dialog_mode {
                    buffer.pop();
                }
                None
            }
    ...
    /// Write the new custom title to the conversation's JSONL file and refresh
    /// the in-memory conversation + search worker so the list reflects the
    /// change immediately.
    fn apply_rename(&mut self, title: String) {
        let path = match &self.app_mode {
            AppMode::View(s) => s.conversation_path.clone(),
            _ => return,
        };
        let Some(session_id) = path.file_stem().and_then(|s| s.to_str()) else {
            self.status_message = Some((
                "Rename failed: cannot derive session id from path".to_string(),
                std::time::Instant::now(),
            ));
            return;
        };
        if let Err(e) = crate::history::rename::write_custom_title(&path, session_id, &title) {
            self.status_message =
                Some((format!("Rename failed: {}", e), std::time::Instant::now()));
            return;
        }

        let new_title = if title.is_empty() { None } else { Some(title) };
        for conv in self.conversations.iter_mut() {
            if conv.path == path {
                conv.custom_title = new_title.clone();
                break;
            }
        }
        self.searchable = search::precompute_search_text(&self.conversations);
    ...
    fn handle_view_key(
        ...
        if self.keys.resume.matches(code, modifiers) {
            return if self.single_file_mode {
                ...
                self.get_selected_path().map(|path| Action::Resume {
                    path,
                    skip_permissions: self.default_skip_permissions,
                })
            };
        }
        if self.keys.resume_alt.matches(code, modifiers) {
            return if self.single_file_mode {
                None
            } else {
                self.get_selected_path().map(|path| Action::Resume {
        ...
        if self.keys.fork.matches(code, modifiers) {
            return if self.single_file_mode {
                ...
                self.get_selected_path().map(|path| Action::ForkResume {
                    path,
                    skip_permissions: self.default_skip_permissions,
                })
            };
        }
        if self.keys.fork_alt.matches(code, modifiers) {
            return if self.single_file_mode {
                None
            } else {
                self.get_selected_path().map(|path| Action::ForkResume {
        ...
        match code {
            ...
            KeyCode::Char('r') if !modifiers.contains(KeyModifiers::CONTROL) => {
                if !self.single_file_mode {
                    ...
                    let current = self
                        ...
                        .iter()
                        .find(|c| c.path == path)
                        .and_then(|c| c.custom_title.clone())
                        .unwrap_or_default();
                    self.dialog_mode = DialogMode::RenameInput { buffer: current };
                }
                None
            }
            ...
            // Toggle questions-only view
            KeyCode::Char('Q') => {
                self.toggle_view_questions_only(viewport_height);
                None
            }
            ...
            // Open clipboard-copy menu (full conversation)
            KeyCode::Char('e') => {
    ...
    fn handle_list_key(
        ...
        if self.keys.resume.matches(code, modifiers) {
            return self.get_selected_path().map(|path| Action::Resume {
                path,
                skip_permissions: self.default_skip_permissions,
            });
        }
        ...
        if self.keys.fork.matches(code, modifiers) {
            return self.get_selected_path().map(|path| Action::ForkResume {
                path,
                skip_permissions: self.default_skip_permissions,
            });
        }
    ...
    /// Toggle questions-only mode in the viewer.
    ///
    /// When entering questions-only mode we also activate message navigation
    /// so the user immediately sees the gutter indicator and can use J/K to
    /// jump between questions. The existing `re_render_view` anchor logic
    /// preserves which message was focused across the toggle, so toggling
    /// back scrolls the full view to the question the user had selected.
    fn toggle_view_questions_only(&mut self, viewport_height: usize) {
        if let AppMode::View(ref mut state) = self.app_mode {
            state.questions_only = !state.questions_only;
            if state.questions_only {
    ...
    pub fn re_render_view(&mut self, viewport_height: usize) {
        use crate::tui::viewer::{RenderOptions, render_conversation};
        ...
        if let AppMode::View(ref mut state) = self.app_mode {
            let options = RenderOptions {
                ...
                questions_only: state.questions_only,
            };
...
pub fn run_with_loader(
    ...
    current_project_dir_name: Option<String>,
    default_skip_permissions: bool,
    default_resume_args: Vec<String>,
    show_last: bool,
    ...
    // `rx` is replaced on each refresh; bind it mutably.
    let mut rx = rx;
    // "Empty history" only bails out the TUI on the very first load. After
    // a refresh the user is already inside the TUI — staying with an empty
    // list is the right behavior (lets them retry, switch scope, etc.).
        ...
        loop {
            match rx.try_recv() {
                ...
                Ok(LoaderMessage::Done) => {
                    ...
                    app.finish_loading();
                    if initial_load {
                        // Initial empty-history check still applies: if the
                        // user has no sessions at all, there's nothing to
                        // do — bail out as before.
                        if count == 0 {
                            drop(guard);
                ...
                Err(std::sync::mpsc::TryRecvError::Disconnected) => {
                    // Loader finished unexpectedly (no Done message). Treat
                    // it like Done: finish loading, apply the initial-load
            ...
            if let Some(action) = app.handle_key(key.code, key.modifiers, viewport_height) {
                match action {
                    ...
                    | Action::ForkResume {
                        ...
                        let fork = matches!(action, Action::ForkResume { .. });
                        let msg = launch_in_new_terminal(
                            &app,
                            path,
                            fork,
                            skip_permissions,
                        ...
                        // After (attempted) launch, return to the list.
                        app.exit_view_mode();
                        // Continue the loop (don't exit TUI)
                    }
                    Action::Refresh => {
                        // If the user is in the viewer, re-render the
                        // current conversation from disk first — this is
                        // the visible "thing" they expect to update,
                        // independent of the list re-scan.
                        if matches!(app.app_mode(), AppMode::View(_)) {
                            app.re_render_view(viewport_height);
                        }
                        // Stash the path to re-select, clear the list, and
                        // mark loading. The next loop iteration will start
                        // pulling batches from the new receiver.
                        app.begin_refresh();
                        rx = crate::history::load_all_conversations_streaming(
                            show_last,
                            debug_level,
                        );
                        app.set_status_message("Refreshing…".to_string());
                        break; // redraw before processing more events
                    }
...
pub fn run_single_file(
    ...
    loop {
        ...
        for ev in events {
            ...
            match app.handle_key(key.code, key.modifiers, viewport_height) {
                Some(Action::Quit) => return Ok(()),
                Some(Action::Refresh) => {
                    // Single-file mode has only one conversation; refresh
                    // = re-render it from disk to pick up newly appended
                    // messages from a running Claude Code session.
                    app.re_render_view(viewport_height);
...
/// Build the resume plan, run any required copy step, and ask the OS to
/// open a new terminal window running `claude --resume <id>`. Returns the
/// status message to surface in the TUI's status bar — either a success
/// string ("Resumed in new terminal window") or an error string.
///
/// Driven only by the in-TUI Ctrl+R / Alt+R / Ctrl+F / Alt+F flow. The
/// `ccmanager --resume` CLI flag uses `launcher::exec_in_place`
/// instead and never reaches here.
fn launch_in_new_terminal(
    app: &App,
    path: &PathBuf,
    fork: bool,
    skip_permissions: bool,
    ...
    let cwd = match std::env::current_dir() {
        Ok(c) => c,
        Err(e) => return format!("Resume failed: cannot read cwd ({})", e),
    };

    let plan = match crate::launcher::plan_resume(
        path,
        project_path,
        default_args,
        fork,
        ...
        Ok(p) => p,
        Err(e) => return format!("Resume failed: {}", e),
    };

    if let Err(e) = crate::launcher::run_copy_step(path, &plan) {
        return format!("Resume failed (copying session files): {}", e);
    }

    match crate::launcher::spawn_in_new_terminal(&plan) {
        Ok(()) => {
            let verb = if fork { "Forked" } else { "Resumed" };
